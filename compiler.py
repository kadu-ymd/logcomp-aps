...
RESERVED_WORDS = ["enter", "program", "end", "move", "interact", "define", "sequence", "conditional", "loop", "while", "up", "right", "down", "left", "locker", "closet", "desk", "exit_door", "locked", "unlocked", "is", "is_not", "object", "else", "if", "true", "false", "collect", "open"]
        ...
        if self.position != len(self.source):
            ...
            if len(val):
                if re.match(VAR_PATTERN, val, flags=0):
                    ...
                    elif val in ["enter", "program", "end", "move", "interact", "define", "sequence", "conditional", "loop", "while", "collect", "object", "else", "if", "open"]:
                        self.next = Token(val, val)
...
class DirectionVal(Node):
    def evaluate(self, st):
        return ("direction", self.value, False)


class ActionVal(Node):
    def evaluate(self, st):
        return ("action", self.value, False)


class CollectableVal(Node):
    def evaluate(self, st):
        return ("collectable", self.value, False)


class InteractableVal(Node):
    def evaluate(self, st):
        return ("interactable", self.value, False)


class StateVal(Node):
    def evaluate(self, st):
        return ("state", self.value, False)


class RelationalBoolVal(Node):
    def evaluate(self, st):
        return ("relational_bool", self.value, False)


class RelationalOperatorVal(Node):
    def evaluate(self, st):
        return ("relational_operator", self.value, False)
...
class CollectableCondition(Node):
    def evaluate(self, st):
        ...
        operator = operator_node.evaluate(st)[1]
...
class ObjectCondition(Node):
    def evaluate(self, st):
        direction_node = self.children[0]
        relational_bool_node = self.children[1]
        object_target_node = self.children[2]
...
class CollectCommand(Node):
    def evaluate(self, st):
        collectable_node = self.children[0]
        direction_node = self.children[1]
...
class Parser:
    ...
    def parse_statement() -> Node:
        ...
        if Parser.tokenizer.next.type not in ["line_break", "program", "end", "EOF"]:
            raise Exception(f"Erro de sintaxe: Statement inválido ou token inesperado. Esperado quebra de linha ou 'program'/'end'/'EOF'. Encontrado '{Parser.tokenizer.next.type}' ('{Parser.tokenizer.next.value}') na posição {Parser.tokenizer.position}.")
    ...
    def parse_movement_block() -> MovementBlock:
        ...
        direction_node = DirectionVal(Parser.tokenizer.next.value, [])
        Parser.tokenizer.select_next()
        value_node = Parser.parse_factor()
        return MovementBlock(None, [direction_node, value_node])

    @staticmethod
    def parse_interact_block() -> InteractBlock:
        Parser.tokenizer.select_next()
        if Parser.tokenizer.next.type not in ["open", "collect"]:
            raise Exception("Erro de sintaxe: Esperado 'open' ou 'collect' após 'interact'")
        action_node = ActionVal(Parser.tokenizer.next.value, [])
        Parser.tokenizer.select_next()
        if Parser.tokenizer.next.type != "direction":
            raise Exception("Erro de sintaxe: Esperado DIREÇÃO após ação de 'interact'")
        direction_node = DirectionVal(Parser.tokenizer.next.value, [])
        Parser.tokenizer.select_next()
        return InteractBlock(None, [action_node, direction_node])
    ...
    def parse_collectable_condition() -> CollectableCondition:
        if Parser.tokenizer.next.type not in ["collectable", "iden"]:
            raise Exception("Erro de sintaxe: Esperado COLLECTABLE (key, coins, card) ou IDENTIFICADOR para COLLECTABLE_CONDITION")
        
        identifier_node = None
        if Parser.tokenizer.next.type == "collectable":
            identifier_node = CollectableVal(Parser.tokenizer.next.value, [])
        else:
            identifier_node = Identifier(Parser.tokenizer.next.value, [])
        ...
        operator_node = RelationalOperatorVal(Parser.tokenizer.next.value, [])

        Parser.tokenizer.select_next()

        value_node = Parser.parse_factor()

        return CollectableCondition(None, [identifier_node, operator_node, value_node])
    ...
    def parse_object_condition() -> ObjectCondition:
        ...
        relational_bool_node = RelationalBoolVal(Parser.tokenizer.next.value, [])
        Parser.tokenizer.select_next()
        if Parser.tokenizer.next.type in ["interactable", "state"]:
            if Parser.tokenizer.next.type == "interactable":
                object_target_node = InteractableVal(Parser.tokenizer.next.value, [])
            else: # state
                object_target_node = StateVal(Parser.tokenizer.next.value, [])
            Parser.tokenizer.select_next()
        else:
            raise Exception("Erro de sintaxe: Esperado INTERACTABLE ou STATE em OBJECT_CONDITION")
        return ObjectCondition(None, [direction_node, relational_bool_node, object_target_node])
    ...
    def parse_collect_command() -> CollectCommand:
        ...
        if Parser.tokenizer.next.type != "collectable":
            raise Exception("Erro de sintaxe: Esperado COLLECTABLE (key, coins, card) após 'collect'")
        
        collectable_node = CollectableVal(Parser.tokenizer.next.value, [])
        ...
        return CollectCommand(None, [collectable_node, direction_node])
...
def main():
    ...
    try:
        ...
        print(f"Compilation successful for {filename}. AST generated.")
